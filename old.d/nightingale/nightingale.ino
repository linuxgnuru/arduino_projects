...
void loop()
{
  if (f_wdt == 1)  // wait for timed out watchdog / flag is set when a watchdog timeout occurs
  {
    ...
    nint++;
    digitalWrite(LED_BUILTIN, 1);  // let led blink
#ifdef SER_DEB
    Serial.print("Sleep ");
    Serial.println(nint);
#endif
    ...
    pinMode(LED_BUILTIN, INPUT); // set all used port to intput to save power
    pinMode(pinPiezoSpeaker, INPUT); // set all used port to intput to save power
    system_sleep();
    pinMode(LED_BUILTIN, OUTPUT); // set all ports into state before sleep
