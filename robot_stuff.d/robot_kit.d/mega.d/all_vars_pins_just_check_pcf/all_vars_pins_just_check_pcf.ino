    ...
    if (hasChanged)
    {
      ...
      if (hasChanged_forward_down || hasChanged_back_down || hasChanged_back_back)
      {
        ...
        else
        {
          ...
          {
            DEBUG_PRINTFLN("fd", hasChanged_forward_down);
          }
          if (hasChanged_back_down)
          {
            DEBUG_PRINTFLN("bd", hasChanged_back_down);
          }
          if (hasChanged_back_back)
          {
            DEBUG_PRINTFLN("bu", hasChanged_back_back);
          }
          DEBUG_PRINTLN("-----");
        }
        sens_hit_flag = false;
...
void blink_extBat()
{
  ...
  if (currentMillis - lastMillisBlink >= interval)
  {
