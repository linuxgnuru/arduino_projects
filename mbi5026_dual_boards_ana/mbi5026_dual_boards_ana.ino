...
void led(int lednum, int cmd, int bnum)
{
  if ((lednum < 0 || lednum > 15) || (cmd < LED_ON || cmd > LED_A_OFF) || (bnum < 0 || bnum > 2))
    return;
  switch (cmd)
  {
    case LED_ON:
      ...
      else
      {
        if (lednum < 8)
        {
          bitSet(data[bnum][0], lednum);
          bitSet(data[bnum][2], lednum);
        }
        else
        {
          lednum = abs(lednum - 8);
          bitSet(data[bnum][1], lednum);
          bitSet(data[bnum][3], lednum);
        }
      }
      break;
    case LED_OFF:
      ...
      else
      {
        if (lednum < 8)
        {
          bitClear(data[bnum][0], lednum);
          bitClear(data[bnum][2], lednum);
        }
        else
        {
          lednum = abs(lednum - 8);
          bitClear(data[bnum][1], lednum);
          bitClear(data[bnum][3], lednum);
        }
      }
      break;
    case LED_TOGGLE:
      ...
      else
      {
        if (lednum < 8)
        {
          bitWrite(data[bnum][0], lednum, !bitRead(data[bnum][0], lednum));
          bitWrite(data[bnum][2], lednum, !bitRead(data[bnum][2], lednum));
        }
        else
        {
          lednum = abs(lednum - 8);
          bitWrite(data[bnum][1], lednum, !bitRead(data[bnum][1], lednum));
          bitWrite(data[bnum][3], lednum, !bitRead(data[bnum][3], lednum));
        }
      }
      break;
    case LED_LE_A_ON: // led_left_all_on
      ...
      else
      {
        for (int i = 3; i < 8; i++)
        {
          bitSet(data[bnum][0], i);
          bitSet(data[bnum][2], i);
        }
      }
      break;
    case LED_RI_A_ON:
      ...
      else
      {
        for (int i = 11; i < 16; i++)
        {
          bitSet(data[bnum][1], i);
          bitSet(data[bnum][3], i);
        }
      }
      break;
    case LED_LE_A_OFF:
      ...
      else
      {
        for (int i = 3; i < 8; i++)
        {
          bitClear(data[bnum][0], i);
          bitClear(data[bnum][2], i);
        }
      }
      break;
    case LED_RI_A_OFF:
      ...
      else
      {
        for (int i = 11; i < 16; i++)
        {
          bitClear(data[bnum][1], i);
          bitClear(data[bnum][3], i);
        }
      }
      break;
    case LED_A_ON:
...
void writeData(int n)
{
  ...
  switch (n)
  {
    case BOARD_SPI:
      ...
      SPI.transfer(SPI_d[0]);
      SPI.transfer(SPI_d[1]);
      SPI.transfer(SPI_d[2]);
    ...
    case BOARD_B:
      ...
      break;
    case BOARD_0: case BOARD_1:
      digitalWrite(dPins[n][LATCH], LOW);
      shiftOut(dPins[n][DATA], dPins[n][CLOCK], MSBFIRST, data[n][0]);
      shiftOut(dPins[n][DATA], dPins[n][CLOCK], LSBFIRST, data[n][1]);
      shiftOut(dPins[n][DATA], dPins[n][CLOCK], MSBFIRST, data[n][2]);
      shiftOut(dPins[n][DATA], dPins[n][CLOCK], LSBFIRST, data[n][3]);
      digitalWrite(dPins[n][LATCH], HIGH);
      break;
    default:
      break;
  }
...
void RunDemo()
{
  ...
  for (int i = 0; i < 3; i++)
  {
    led(l_r[0][i], LED_ON, BOARD_0);
    led(l_r[0][i], LED_ON, BOARD_1);
    delay(DL);
    led(l_r[1][i], LED_ON, BOARD_0);
    led(l_r[1][i], LED_ON, BOARD_1);
    delay(DL);
  }
  for (int i = 3; i < 8; i++)
  {
    led(i, LED_OFF, BOARD_B);
    delay(DL);
  }
  for (int i = 15; i > 10; i--)
  {
  ...
  for (int i = 2; i > -1; i--)
  {
    led(l_r[0][i], LED_OFF, BOARD_0);
    led(l_r[0][i], LED_OFF, BOARD_1);
    delay(DL);
    led(l_r[1][i], LED_OFF, BOARD_0);
    led(l_r[1][i], LED_OFF, BOARD_1);
    delay(DL);
  }
}
...
void parseData(int v, boolean l_r, int bnum)
{
  int d_a, d_b;
  int myV;
  //void led(int lednum, int cmd, int bnum)

  if (bnum < BOARD_0 || bnum > BOARD_SPI)
    return;
  // set lednum to anything; it won't matter
  if (v == 0 || v == 5)
  {
    if (v == 0)
      led(0, (!l_r ? LED_LE_A_OFF : LED_RI_A_OFF), bnum);
    else
      led(0, (!l_r ? LED_LE_A_ON : LED_RI_A_ON), bnum);
  }
  else
  {
