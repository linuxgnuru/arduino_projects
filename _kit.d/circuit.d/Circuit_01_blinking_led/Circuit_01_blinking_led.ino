/*
NOTE: pin 13 changes from one board to the next, so they have added
      a #define of LED_BUILTIN which represents the correct pin.
...
Hardware connections:
  ...
  pin LED_BUILTIN, so you may not need any additional circuitry.

  But if you'd like to connect a second LED to pin LED_BUILTIN, or use
  a different pin, follow these steps:

    Connect the positive side of your LED (longer leg) to Arduino
    digital pin LED_BUILTIN (or another digital pin, don't forget to change
    the code to match).
    ...
    pin LED_BUILTIN _____ + LED - _____ 330 Ohm _____ GND
...
void setup()
{
  // The Arduino has LED_BUILTIN digital input/output pins. These pins
  ...
  // Here we'll set up pin LED_BUILTIN (the one connected to a LED) to be
...
void loop()
{
  // The LED_BUILTIN digital pins on your Arduino are great at inputting
  ...
  // Because we have an LED connected to pin LED_BUILTIN, if we make that
